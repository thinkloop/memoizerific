 ...
 * MIT license.
 */
(function(root, factory) { if (typeof define === 'function' && define.amd) { define([], factory); } else if (typeof exports === 'object') { module.exports = factory(); } else { root.memoize = factory(); }}(this, function() {"use strict";
        		...
        		// if args not yet cached and result not yet memoized, build the missing parts of the cache tree
        			...
        			// if we are at the last arg, run the fn to get the result to memoize
        				...
        				if (limit && limit > 0) {
            				lru.push(args);
            				if (lru.length > limit) {
            				    removeCachedResult(cache, lru.shift());
            				}
        				}
        			}
        		}
...
function moveToMostRecentLru(lru, args) {
...
function removeCachedResult(cache, removedLru) {
